...
class RunsWithState m where
    ...
    runWithState :: m a -> MonadStateType m -> Manager -> IO (Either ServantError a)

instance RunsWithState ApiAiClient where
    type MonadStateType ApiAiClient = ApiAiClientState
    runWithState acm st manager = runClient' acm st (ClientEnv manager apiAiBaseUrl)

instance RunsWithState ApiAiDeveloper where
    type MonadStateType ApiAiDeveloper = ApiAiDeveloperState
    runWithState adm st manager = runDeveloper' adm st (ClientEnv manager apiAiBaseUrl)
...
runWithToken am t = newManager tlsManagerSettings >>= runWithState am (withToken t)
